...
let
  ...
  in coercedTo pkgType coercePkg (submodule {
    options = {
      pkg = mkOption {
  ...
  callAttrPackage = pkgs: { pkg, override, overrideAttrs, ... }: let
    ...
    pkg'' = if isOverridden then pkg'.override override' else pkg'; in 
  ...
  borg-service =
    ...
    , subdir
    , ... }: let
      python-pkg = python.withPackages (ps: let
